                                ...
                                <table id="example1" class="table table-bordered table-striped">
                                    ...
                                    @forelse($datas as $data)
                                        <tr id="data-{{$data->id}}">
                                            ...
                                            <td>
                                                <a href="{{  route('user.index',['country_id'=>$data->id]) }}"
                                                   class="btn btn-outline-primary btn-block btn-sm">{{$custom[strtolower('user')]??""}}{{" - " .$data->user->count()}}</a>
                                                <a href="{{  route('city.index',['country_id'=>$data->id]) }}"
                                                   class="btn btn-outline-primary btn-block btn-sm">{{$custom[strtolower('city')]??""}}{{" - " .$data->city->count()}}</a>
                                                <a href="{{  route('state.index',['country_id'=>$data->id]) }}"
                                                   class="btn btn-outline-primary btn-block btn-sm">{{$custom[strtolower('state')]??""}}{{" - " .$data->state->count()}}</a>
                                                <a href="{{  route('task.index',['country_id'=>$data->id]) }}"
                                                   class="btn btn-outline-primary btn-block btn-sm">{{$custom[strtolower('task')]??""}}{{" - " .$data->task->count()}}</a>
                                                <a href="{{  route('ad.index',['country_id'=>$data->id]) }}"
                                                   class="btn btn-outline-primary btn-block btn-sm">{{$custom[strtolower('ad')]??""}}{{" - " .$data->ad->count()}}</a>
                                            </td>
                                            <td>
