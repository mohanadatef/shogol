                                ...
                                <table id="example1" class="table table-bordered table-striped">
                                    ...
                                    @forelse($datas as $data)
                                        <tr id="data-{{$data->id}}">
                                            ...
                                            <td>
                                                <a href="{{  route('ad.index',['category'=>$data->id]) }}"
                                                   class="btn btn-outline-primary btn-block btn-sm">{{$custom[strtolower('ad')]??""}}{{" - " .$data->ad->count()}}</a>
                                                <a href="{{  route('task.index',['category'=>$data->id]) }}"
                                                   class="btn btn-outline-primary btn-block btn-sm">{{$custom[strtolower('task')]??""}}{{" - " .$data->task->count()}}</a>
                                                <a href="{{  route('user.index',['category'=>$data->id]) }}"
                                                   class="btn btn-outline-primary btn-block btn-sm">{{$custom[strtolower('user')]??""}}{{" - " .$data->user->count()}}</a>
                                                <a href="{{  route('tag.index',['category_id'=>$data->id]) }}"
                                                   class="btn btn-outline-primary btn-block btn-sm">{{$custom[strtolower('tag')]??""}}{{" - " .$data->tags->count()}}</a>
                                                <a href="{{  route('category.index',['parent_id'=>$data->id]) }}"
                                                   class="btn btn-outline-primary btn-block btn-sm">{{$custom[strtolower('parent')]??""}}{{" - " .$data->children->count()}}</a>
                                            </td>
                                            <td>
