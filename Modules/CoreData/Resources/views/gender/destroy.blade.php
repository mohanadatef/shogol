    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1>{{$custom[strtolower('Gender')]??"lang not found"}}</h1>
                    </div>
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="{{route('admin.dashboard')}}">{{$custom[strtolower('Home')]??"lang not found"}}</a></li>
                            <li class="breadcrumb-item active">{{$custom[strtolower('Gender')]??"lang not found"}}</li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
                                ...
                                <table id="example1" class="table table-bordered table-striped">
                                    ...
                                    @forelse($datas as $data)
                                        <tr id="data-{{$data->id}}">
                                            ...
                                            <td>
                                                <a href="{{  route('user.index',['gender_id'=>$data->id]) }}"
                                                   class="btn btn-outline-primary btn-block btn-sm"></i>{{$custom[strtolower('user')]??""}}{{" - " .$data->user->count()}}</a>
                                            </td>
                                            <td>
