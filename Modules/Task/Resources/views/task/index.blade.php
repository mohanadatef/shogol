    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            <form method="get" action="">
                <div class="container-fluid">
                @permission('task-filter')
                @include('task::task.filter')
                @endpermission
                    <div class="row">
                        <div class="col-12">
                            <div class="card">
                                    ...
                                    <table id="example1" class="table table-bordered table-striped">
                                        ...
                                        @forelse($datas as $data)
                                            <tr id="data-{{$data->id}}">
                                                ...
                                                <td id="category_id-{{$data->id}}"> @foreach($data->category as $category)
                                                        <br>
                                                        {{$category->name->value ?? ""}}
                                                    @endforeach</td>
                                                @permission('task-change-status')
                                                <td id="status-{{$data->id}}">{{$custom[strtolower('Status')]??"lang not found"}}<a href="{{  route('status.index',['id'=>$data->status_id]) }}">
                                                    ...
                                                    <br>
                                                        ...
                                                        <button type="button"
                                                            ...
                                                            <i></i> {{$custom[strtolower('unapprove')]??"lang not found"}}
                                                        </button>
                                                        ...
                                                        @endpermission
                                                    @elseif($data->status_id == statusType()['us'])
                                                        <br>
                                                    ...
                                                    @endif
                                                    @if(in_array($data->status_id,[statusType()['ns'],statusType()['os'],statusType()['as'],statusType()['ts']]))
                                                        @permission('task-cansel')
                                                    <br>
                                                        <button type="button"
                                                                class="btn btn-outline-danger btn-flat btn-sm"
                                                ...
                                                @permission('task-show')
                                                    <a href="{{  route('task.show',$data->id) }}"
                                                       class="btn btn-outline-primary btn-block btn-sm"><i class="fa fa-edit"></i>{{$custom[strtolower('open')]??""}}</a>
                                                       @endpermission
            ...
            <div class="modal-content bg-info">
                ...
                <form id="reject" action="" method="post" name="reject" enctype="multipart/form-data">
                    ...
                    <div class="modal-footer justify-content-between">
                        ...
                        <button type="submit" class="btn btn-outline-light">{{$custom[strtolower('reject')]??"lang not found"}}</button>
                    </div>
                </form>
            </div>
            <!-- /.modal-content -->
