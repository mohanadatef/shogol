    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1>{{$custom[strtolower('FQ')]??"lang not found"}}</h1>
                    </div>
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="{{route('admin.dashboard')}}">{{$custom[strtolower('Home')]??"lang not found"}}</a></li>
                            <li class="breadcrumb-item active">{{$custom[strtolower('FQ')]??"lang not found"}}</li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
                            ...
                            @permission('fq-create')
                                ...
                                <div class="card-body">
                                    <table id="example1" class="table table-bordered table-striped">
                                        <thead>
                                        <tr>
                                            <th>{{$custom[strtolower('Question')]??"lang not found"}}</th>
                                            <th>{{$custom[strtolower('Answer')]??"lang not found"}}</th>
                                            @permission('fq-change-status')
                                        ...
                                        @forelse($datas as $data)
                                            <tr id="data-{{$data->id}}">
                                                <td id="question-{{$data->id}}" data-order="{{$data->order}}">{{$data->question->value ??""}}</td>
                    ...
                    <div class="modal-body">
                        <div class="card-body">
                            @foreach($language as $lang)
                                <div class="form-group{{ $errors->has('question['.$lang->code.']') ? ' is-invalid' : "" }}">
                                    <label for="question">{{$custom[strtolower('question')]??"lang not found"}} {{$lang->name}}</label>
                                    <input type="text" name="question[{{$lang->code}}]" class="form-control"
                                           id="question[{{$lang->code}}]"
                                           value="{{Request::old('question['.$lang->code.']')}}"
                                           placeholder="{{$custom[strtolower('Enter_question')]??"lang not found"}} {{$lang->name}}">
                                </div>
                                <div class="form-group{{ $errors->has('answer['.$lang->code.']') ? ' is-invalid' : "" }}">
                                    <label for="answer">{{$custom[strtolower('answer')]??"lang not found"}} {{$lang->name}}</label>
                                    <input type="text" name="answer[{{$lang->code}}]" class="form-control"
                                           id="answer[{{$lang->code}}]"
                                           value="{{Request::old('answer['.$lang->code.']')}}"
                                           placeholder="{{$custom[strtolower('Enter_answer')]??"lang not found"}} {{$lang->name}}">
                                </div>
                            @endforeach
                        ...
                        <div class="card-body">
                            @foreach($language as $lang)
                                <div class="form-group{{ $errors->has('question['.$lang->code.']') ? ' is-invalid' : "" }}">
                                    ...
                                    <input type="text" name="question[{{$lang->code}}]" class="form-control"
                                           id="question-{{$lang->code}}"
                                           value="" placeholder="{{$custom[strtolower('Enter_question')]??"lang not found"}} {{$lang->name}}">
                                </div>
                                <div class="form-group{{ $errors->has('answer['.$lang->code.']') ? ' is-invalid' : "" }}">
                                    <label for="answer">{{$custom[strtolower('answer')]??"lang not found"}} {{$lang->name}}</label>
                                    <input type="text" name="answer[{{$lang->code}}]" class="form-control"
                                           id="answer-{{$lang->code}}"
                                           value="" placeholder="{{$custom[strtolower('Enter_answer')]??"lang not found"}} {{$lang->name}}">
                                </div>
                            @endforeach
                            <div class="form-group{{ $errors->has('order') ? ' is-invalid' : "" }}">
                                <label for="order">{{$custom[strtolower('Order')]??"lang not found"}}</label>
